...
public class MainController {
    ...
    private static final Logger LOGGER = util.LoggerConfig.getLogger();

    private final service.ExchangeRateApiService api;
    private final Scanner scanner;
    private final view.MenuView menuView;
    private model.ExchangeRateResponse rates;

    public MainController() {
        this.api = new service.ExchangeRateApiService();
        this.scanner = new Scanner(System.in);
        this.menuView = new view.MenuView();
    }
    ...
    private void ejecutarMenu() {
        ...
        Map<Integer, Runnable> acciones = util.ConversionActions.build(fromTo);
        final String SEPARATOR = BLUE + "=".repeat(60) + RESET;
        boolean ejecutando = true;

        while ( ejecutando) {
            menuView.mostrarMenu();
            int option = util.ConsoleUtils.leerOpcion(scanner, "");

            if (option == OPCION_LISTAR_DIVISAS) {
                mostrarTodasLasDivisas();
            } else if (option == OPCION_SALIR) {
                util.ConsoleUtils.mostrarDespedida("👋 ¡Hasta luego y gracias por usar el conversor!", SEPARATOR, GREEN, RESET);
                ejecutando = false;
            } else {
    ...
    private void procesarConversion(int option, String[] fromTo, Map<Integer, Runnable> acciones, String SEPARATOR) {
        double amount = util.ConsoleUtils.leerMonto(scanner, YELLOW + "💰 Ingrese el monto a convertir: " + RESET);
        ...
        if (accion != null) {
            ...
            util.ConsoleUtils.mostrarSeparador(SEPARATOR);

            double result = service.CurrencyConverter.convertirMoneda(amount, from, to, rates);
            util.ConsoleUtils.mostrarResultado(amount, from, result, to, GREEN + "\uD83D\uDCB6 Resultado: " + CYAN);
            util.ConsoleUtils.mostrarSeparador(SEPARATOR);
        } else {
            util.ConsoleUtils.mostrarError("❌ Opción no válida, por favor seleccione entre 1 y 8.", RED);
        }
    }
